...
3. ~~**LDA на 5 темах** — число выбрано эвристически.~~ ✅ Исправлено в секции 11 —
   теперь используется автоматический подбор K по Coherence Score.

4. **Категориальные дамми (`category_grouped`)** подхватываются только в
   `econometrics.py`. В ML-скриптах они не используются — добавить в `CONTROLS` при
   необходимости.
...
7. **Юникодные графики (kириллические подписи)** не настраиваются специально —
   matplotlib по умолчанию не имеет шрифта с кириллицей на некоторых системах. Если
   подписи ломаются, добавить `plt.rcParams["font.family"] = "DejaVu Sans"`.
