...
Словари (все слова = pymorphy3 normal_form):
  CERTAINTY_WORDS (~30 лемм):
    "точно", "конечно", "обязательно", "безусловно", "несомненно",
    "определённо", "уверенный", "убеждённый", "гарантировать",
    "доказать", "проверить", "подтвердить", "выполнимый", "факт", ...

  UNCERTAINTY_WORDS (~25 лемм):
    "возможно", "вероятно", "наверное", "если", "пожалуй",
    "мочь", "думать", "надеяться", "планировать", "хотеть",
    "попробовать", "ожидать", "предполагать", "риск", "шанс", ...
...
✅ ИСПРАВЛЕНО: словари в dicts.py приведены к pymorphy3 normal_form.
  Покрытие после исправления: certainty 48%, uncertainty 90%, social 100%.
  Подробности — см. §13, пункт 8.
```
...
Текст разбивается на предложения, каждое — до 512 токенов, батч 16, softmax по логитам.
Итог — среднее распределение вероятностей `[positive, negative, neutral]` по всем предложениям документа.

**Сравнение моделей тональности (используется одна; остальные — альтернативы):**

| Модель | Особенность | Статус |
|---|---|---|
| `blanchefort/rubert-base-cased-sentiment` | полноразмерный RuBERT, обучен на reviews/tweets | **текущая** |
| `seara/rubert-tiny2-russian-sentiment` | дистиллированная (~28M параметров), быстрее в ~4×, точность чуть ниже | альтернатива |
| `MonoHime/rubert-base-cased-sentiment-new` | полноразмерный RuBERT, другой обучающий датасет | альтернатива |
...
**Примечание о покрытии словарей:** После `extract_features.py` запускается проверка
в `extract_features.py::validate_dictionary_coverage()`, которая показывает, какой % текстов
содержит слова из словарей. Текущее покрытие (N=100): `social_score` 100%,
`certainty_score` 48%, `uncertainty_score` 90%, `gratitude_score` 49%.
...
```python
# В prepare():
cat_dummies = pd.get_dummies(df["category_grouped"], prefix="cat", drop_first=False)
cat_dummies = cat_dummies.drop(columns=["cat_Other"], errors="ignore")
# Результат: cat_Бизнес, cat_Досуг и события, cat_Медиа и контент,
#            cat_Образование и просвещение, cat_Социальные, cat_Творческие, cat_Технологии
```

Дамми включаются в набор A и C для захвата влияния категории проекта на успешность:
- Одни категории могут быть систематически более успешны чем другие
- Категория — potential confounding factor для текстовых признаков

**Базовая категория: `Other`** (проекты с пустой или нераспознанной категорией).

Выбор обоснован: `Other` — бессодержательный остаток, а не одна из семи тематических
категорий. Коэффициенты `cat_Бизнес`, `cat_Творческие`, … интерпретируются как отклонение
от проектов без категории, что осмысленно для анализа.

Все 7 именованных категорий присутствуют в модели как отдельные дамми.
...
4. ~~**Категориальные дамми (`category_grouped`)** подхватываются только в
   `econometrics.py`.~~ ✅ Исправлено — `ml_models.py` и `econometrics.py` создают one-hot
   дамми в `prepare()` с явным дропом `cat_Other` (базовая категория — `Other`).
...
8. ~~**Низкое покрытие словарей `certainty_score` и `uncertainty_score`**~~ ✅ ИСПРАВЛЕНО

   **Была проблема:** 32.3% текстов содержали certainty слова, 51.5% — uncertainty слова.
   Причина: мисмэтч между `lemmatize_tokens()` и словарём (словари содержали склонённые
   формы вместо pymorphy3 `normal_form`).

   **Решение:** Все три словаря в `dicts.py` (`CERTAINTY_WORDS`, `UNCERTAINTY_WORDS`,
   `SOCIAL_WORDS`) приведены к pymorphy3 `normal_form`:
   - Убраны склонённые формы: `"уверен"` → `"уверенный"`, `"думаю"` → `"думать"`, и т.д.
   - Убраны дубли: `"партнер"` / `"партнёр"` → только `"партнёр"` (normal_form)
   - Множественное число → единственное: `"друзья"` → `"друг"`, `"дети"` → `"ребёнок"`

   **Покрытие после исправления (N=100):**

   | Словарь | Было | Стало |
   |---|---|---|
   | `certainty_score` | 32.3% | 48.0% |
   | `uncertainty_score` | 51.5% | 90.0% |
   | `social_score` | 73.5% | 100.0% |
